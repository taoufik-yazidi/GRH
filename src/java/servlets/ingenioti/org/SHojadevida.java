...
public class SHojadevida extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (SUtilidades.autenticado(sesion)) {
            ...
            String slugarnacimiento = request.getParameter("lugarnacimiento");
            String slugarexpediciond = request.getParameter("lugarexpediciond");
            String slugarresidencia = request.getParameter("lugarresidencia");
            ...
            try {
                ...
                System.err.println("Error al convertir: _fechanacimiento en fecha en el Servlet SHojadevida");
            }
            Calendar cfechaexpediciond = new GregorianCalendar();
            try {
                cfechaexpediciond.setTime(sdf.parse(sfechaexpediciond));
            } catch (ParseException pe) {
                System.err.println("Error al convertir: _fechaexpediciond en fecha en el Servlet SHojadevida");
            }
            ...
            OHojadevida oHojadevida = new OHojadevida(iidhojadevida, iidtipodedocumento, snumerodocumento, sprimerapellido, ssegundoapellido, snombres, iidgenero, slibretamilitar, sdistritolm, ilugarnacimiento, ilugarexpediciond, ilugarresidencia, cfechanacimiento, cfechaexpediciond, iidestadocivil, bdisponibilidadviaje, sdireccion, stelefono, scorreo, sfoto);
            ...
            if (sAccion > 0 && sAccion < 4) {
                ...
                if (sAccion != 3 && (sidtipodedocumento == null || sidtipodedocumento.length() == 0 || snumerodocumento == null || snumerodocumento.length() == 0 || sprimerapellido == null || sprimerapellido.length() == 0 || ssegundoapellido == null || ssegundoapellido.length() == 0 || snombres == null || snombres.length() == 0 || sidgenero == null || sidgenero.length() == 0 || slibretamilitar == null || slibretamilitar.length() == 0 || sdistritolm == null || sdistritolm.length() == 0 || slugarnacimiento == null || slugarnacimiento.length() == 0 || slugarexpediciond == null || slugarexpediciond.length() == 0 || slugarresidencia == null || slugarresidencia.length() == 0 || sfechanacimiento == null || sfechanacimiento.length() == 0 || sfechaexpediciond == null || sfechaexpediciond.length() == 0 || sidestadocivil == null || sidestadocivil.length() == 0 || sdisponibilidadviaje == null || sdisponibilidadviaje.length() == 0 || sdireccion == null || sdireccion.length() == 0 || stelefono == null || stelefono.length() == 0 || scorreo == null || scorreo.length() == 0 || sfoto == null || sfoto.length() == 0)) {
                    tipoMensaje = 4;
            ...
            if (sAccion == 4) { // Consultar
                ...
                for (OHojadevida obj : lista) {
                    JsonObject temp = Json.createObjectBuilder()
                            ...
                            .add("idgenero", obj.getGenero().getGenero())
                            .add("libretamilitar", obj.getLibretamilitar())
                            .add("distritolm", obj.getDistritolm())
                            .add("lugarnacimiento", obj.getLugarnacimiento().getNombre())
                            .add("lugarexpediciond", obj.getLugarexpedicion().getNombre())
                            .add("lugarresidencia", obj.getLugarresidencia().getNombre())
